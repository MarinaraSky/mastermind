                ...
                switch (argv[i][1])
                {
                    case (104):    // case to check if -h is set
                        ...
                        printf("Optional file .mm can be in the same directory.\n"
                            "If .mm is used the -m flag requirements still" " apply.\n");
                        goto end;
    ...
    char *mm_string)
{
        ...
        // counts the first match as white
    {
        for (int j = 0; j <= 4; j++)
        {
            if (temp_guess[i] == temp_mm_string[j] && i != j && 0 != temp_mm_string[j])
            {
                if (last_count == count)
                {
                    count++;
    ...
    char switches)
{
    ...
    if (!(switches & MULTI))    // Bitwise anding to check if multi flag is on
    {
        for (int i = 0; i < 4; i++)
        {
            for (int j = i + 1; j <= 4; j++)
            {
                if (mm_string[i] == mm_string[j] || 48 > mm_string[i] || 57 < mm_string[i])
                {
                    return_code = 0;
                }
            }
